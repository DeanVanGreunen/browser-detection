...
function getPlatformData(userAgent){
    ...
    if(userAgent.includes("Windows NT")){ // is windows
        ...
        info.name = "Mac OS"; //Mac OS X 10_15_7
        let versionString = userAgent.substr(userAgent.indexOf("Mac OS") + 7);
        let skippedOSVerson = versionString.substr(versionString.indexOf(" ") + 1);
        let nextComma = skippedOSVerson.indexOf(",");
        let nextBracket = skippedOSVerson.indexOf(")");
        let versionEncoded = "";
        if(nextBracket < nextComma){
            versionEncoded = skippedOSVerson.substr(0, nextBracket);
        } else {
            versionEncoded = skippedOSVerson.substr(0, nextComma);
        }
        let version = versionEncoded.replace(/_/g,".");
...
function getIsMobile(userAgent){   
    ...
    if(check == false){return navigator?.userAgentData?.mobile;} else {
        return false;
    }
    return check;
}
